...

        {
            let mut pass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
                ...
                color_attachments: &[Some(wgpu::RenderPassColorAttachment {
                    ...
                    resolve_target: None,
                    ops: wgpu::Operations {
                        // `clear_color` defaults to TRANSPARENT (desktop
                        // compositors blend gpui with chrome behind it);
                        // Android sets an opaque color via set_clear_color
                        // so the first frame doesn't flash SurfaceView black.
